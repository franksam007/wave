  ...
  XTable = ({ model: m }: { model: Table }) => {
    const
      ...
      reset = React.useCallback(() => {
        ...
        filter(null)
        search()
      }, [filter, m.events, m.name, m.pagination, search]),
        ...
        setGroups(groups => {
          if (groups) {
            setFilteredItems(filteredItems => groups?.reduce((acc, group) =>
              [...acc, ...filteredItems.slice(group.startIndex, acc.length + group.count).sort(sortingF(selectedSorts))],
              [] as any[]) || [])
          }
    ...
    return (
      <div data-test={m.name} style={{ position: 'relative', height: computeHeight() }}>
        <Fluent.Stack horizontal horizontalAlign='space-between' verticalAlign='end'>
          {m.groupable && !m.pagination && <Fluent.Dropdown data-test='groupby' label='Group by' selectedKey={groupByKey} onChange={onGroupByChange} options={groupByOptions} styles={{ root: { width: 300 } }} />}
